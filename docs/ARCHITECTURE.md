...
Summary reviews include a hidden `<!-- code-review-bot -->` marker. If a marked bot review already exists for the same PR head SHA, the reporter skips posting another review for that head to reduce repeated-test noise. Findings that cannot be placed inline are grouped by category in the summary review, and empty categories are hidden.
